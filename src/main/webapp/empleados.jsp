			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h1 class="modal-title form-head text-bold fs-5" id="modalEmpleadoLabel">Agregar Usuario</h1>
					</div>
						...
						<form id="FormularioEmpleado" method="post"
							action="ServletEmpleados?accion=grabar">
									...
									<div class="form-group">
										<label for="" class="form-label">UsuarioID</label> <input
									...
									<div class="form-group">
										<label for="login" class="form-label">Username</label> <input
											type="text" class="form-control" name="username" id="id-username">
									</div>
									...
									<div class="form-group">
										<label for="contraseña" class="form-label">Activo</label>
...
<script>
    ...
    function cargarEmpleados() {
        $.get("ServletEmpleadosJSON", function(response) {
            ...
            $.each(response, function(index, item) {
                $("#TablaEmpleados").append("<tr><td>" + item.UsuarioID + "</td>" +
                        "<td>" + item.Nombre + "</td>" +
                        "<td>" + item.Username + "</td>" +
                    "<td>" + item.CorreoElectronico + "</td><td>" + item.Rol + "</td>" +
                    "<td>" + item.Activo + "</td><td>" + botonEditar + "</td><td>" + botonEliminar + "</td></tr>");
            });
    ...
    $(document).on("click", ".btn-editar", function() {
        ...
        $.get("ServletFindEmpleadosJSON?id=" + id, function(response) {
            $("#id-codigo").val(response.UsuarioID);
            $("#id-nombre").val(response.Nombre);
            $("#id-username").val(response.Username);
            $("#id-login").val(response.CorreoElectronico);
            $("#id-rol").val(response.Rol);
            $("#id-activo").val(response.Activo);
            $("#id-contraseña").val("******");
